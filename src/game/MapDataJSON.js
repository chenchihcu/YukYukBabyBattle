...
// ===== 依主題取得地面層 Tile 代號 =====
export function getFloorTileForTheme(themeName) {
  switch (themeName) {
    case 'VILLAGE': return TILE_EXT.GRASS;
    case 'RUIN':    return TILE_EXT.STONE_FLOOR;
    case 'SNOW':    return TILE_EXT.SNOW_FLOOR;
    case 'FACTORY': return TILE_EXT.METAL_FLOOR;
    case 'TEMPLE':  return TILE_EXT.LAVA_ROCK;
    default:        return TILE_EXT.GRASS;
  }
}

// ===== Tile 碰撞性判定 =====
// 回傳 true 表示此 Tile 阻擋坦克通行
export function isObstacleSolid(tileVal) {
  // 磚牆、鋼片、水域、基地、木樁欄、沙袋 = 不可通行
  return [1, 2, 3, 6, 7, 14, 15, 42].includes(tileVal);
}

// 回傳 true 表示此 Tile 可被普通子彈摧毀
export function isTileDestructible(tileVal) {
  return [1, 13, 14, 40, 41, 42].includes(tileVal);
}

// 回傳 true 表示此 Tile 在 obstacle 層為可破壞物件
export function isProp(tileVal) {
  return [40, 41, 42, 43].includes(tileVal);
}

// ===== 單一關卡 JSON 格式範本（空白模板）=====
export function createEmptyStageJSON(stageNum, rows = 26, cols = 26) {
  const { name: themeName } = getThemeForStage(stageNum);
  const floorTile = getFloorTileForTheme(themeName);
  return {
    stage: stageNum,
    theme: themeName,
    rows,
    cols,
    floor: Array.from({ length: rows }, () => Array(cols).fill(floorTile)),
    obstacle: Array.from({ length: rows }, () => Array(cols).fill(0)),
    roof: Array.from({ length: rows }, () => Array(cols).fill(0)),
  };
}
